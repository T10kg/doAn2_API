...
function checkmavach_QLXK() {
    ...
    if ($row) {
        ...
        if ($("#check_mavach").is(":checked")) {
            $('.qlxk-qr,.qlxk-qr_cn,.qlxk-mavach_cn ').hide()
            $('.qlxk-mavach').show()
            $('#qlxk-mavach').focus()
        } else {
            $('.qlxk-qr').show()
            $('.qlxk-mavach,.qlxk-qr_cn,.qlxk-mavach_cn ').hide();
            $('#qlxk-qr').focus()
        }
    }
}
...
function capnhatthongtinspxk() {
    ...
    if (row) {
        ...
        if ($soluong > 0 && isNaN($soluong) == !1 && $chietkhau != "") {
            $bien = {
                id: $id,
                soluong: $soluong,
                chietkhau: $chietkhau,
...
function capnhatghichupx() {
    ...
    if (row) {
        ...
        $mapx = row.id
        $("#ghichu_capnhat").textbox("setValue", row.ghichu)
...
function capnhatphieuttvechuaxuat() {
    var a = $("#qlxk-dg").datagrid("getSelected");
    if (a) {
        $mapx = a.id;
        $bien = {
            mapx: $mapx
        };
        if (0 != a.tongthanhtoan)
            return thongbao("Ch\u1ec9 c\u1eadp nh\u1eadt khi t\u1ed5ng thanh=0"),
            !1;
...
;function xuatphieulapkehoachsanxuat() {
    ...
    window.open($h, "_blank")) : thongbao("Ch\u1ecdn phi\u1ebfu xu\u1ea5t c\u1ea7n xu\u1ea5t file")
}
